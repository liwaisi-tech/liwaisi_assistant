...
// goldenTranscriptAlpine is the expected AwakeningReport for a canonical
// Alpine-3.21 + busybox + tar + wget environment (AC-002). Loaded inline so
// Go test tooling does not need a testdata/ file-access step; the spec's
// testdata/ directory lives under cpn/testdata/awakening for integration
// fixtures that DO need the file system (LLM transcripts, etc.).
var goldenTranscriptAlpine = []byte(`{
