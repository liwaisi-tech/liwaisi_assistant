...
func TestFireLLM_ContextWindowAssembly(t *testing.T) {
	...
	_, err := fireLLM(context.Background(), trans, cpn, consumed)
	if err != nil {
...
func TestFireLLM_ContextCancellation(t *testing.T) {
	...
	_, err := fireLLM(ctx, trans, cpn, consumed)
...
func TestFireLLM_HistoryAppendIsThreadSafe(t *testing.T) {
	...
	for i := range goroutines {
		...
		go func(idx int) {
			...
			if _, err := fireLLM(context.Background(), transitions[tID], c, consumed); err != nil {
