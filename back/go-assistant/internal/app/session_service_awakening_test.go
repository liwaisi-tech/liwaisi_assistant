...
// SC-16 — BRAE_AWAKENING_MODE unset selects the default fanout path. We
// assert this by observing the awakening.mode.fanout event is emitted.
func TestRunAwakening_DefaultModeFanout(t *testing.T) {
	t.Setenv("BRAE_AWAKENING_MODE", "")
	restore := stubHostIDResolver(t, "machine-default-mode")
	defer restore()
	...
	_, _, _, _ = svc.runAwakening(context.Background(), "sess-default-mode", "user-test")
	logs := buf.String()
	if !strings.Contains(logs, "brae.awakening.mode.fanout") {
		t.Errorf("expected mode.fanout event; logs=%s", logs)
	}
	if strings.Contains(logs, "brae.awakening.mode.legacy") {
		t.Errorf("unset env MUST NOT emit mode.legacy; logs=%s", logs)
	}
}

// SC-16 — BRAE_AWAKENING_MODE=legacy routes through the single-turn legacy
// topology. The LLM returns a valid AwakeningReport JSON directly; the
// report is projected, persisted, and rendered as a first-turn A2UI envelope.
func TestRunAwakening_LegacyMode(t *testing.T) {
	t.Setenv("BRAE_AWAKENING_MODE", "legacy")
	restore := stubHostIDResolver(t, "machine-legacy-mode")
	defer restore()
	...
	reportJSON := `{"os":{"name":"linux","version":"22.04","kernel":"6.1","arch":"amd64"},` +
		...
		`"narrative_md":"legacy boot","probe_trace":[]}`
	...
	svc := &SessionService{
		...
		llm:                llm,
		awakensFactory: func(_ string, _ awakens.Deps) *cpn.CPN {
			t.Fatal("legacy mode must NOT invoke the fanout factory")
			return nil
		},
	}

	snap, envelope, source, err := svc.runAwakening(context.Background(), "sess-legacy-mode", "user-legacy")
	if err != nil {
		t.Fatalf("runAwakening legacy: %v", err)
	}
	if calls.Load() != 1 {
		t.Errorf("expected exactly 1 LLM call in legacy mode; got %d", calls.Load())
	}
	if source != awakens.SourceAwakening {
		t.Errorf("source = %q; want %q", source, awakens.SourceAwakening)
	}
	if snap.HostID != "machine-legacy-mode" {
		t.Errorf("snap.HostID = %q; want machine-legacy-mode", snap.HostID)
	}
	if len(envelope.Components) == 0 {
		t.Fatal("legacy mode must emit a first-turn A2UI envelope")
	}
	logs := buf.String()
	if !strings.Contains(logs, "brae.awakening.mode.legacy") {
		t.Errorf("expected mode.legacy event; logs=%s", logs)
	}
	got, err := repo.LatestForHost(context.Background(), "machine-legacy-mode")
	if err != nil {
		t.Fatalf("repo.LatestForHost: %v", err)
	}
	if got.Source != awakens.SourceAwakening {
		t.Errorf("persisted source = %q; want %q", got.Source, awakens.SourceAwakening)
	}
}
